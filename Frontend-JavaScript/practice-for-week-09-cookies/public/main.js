...
function getCookie(cookieName) {
  let splitSemi = document.cookie.split("; ");
  let cookie = splitSemi.find((cookie) => cookie.startsWith(cookieName + "="));
...
function restoreTheme() {
  ...
  let cookie = getCookie("themeName");
...
function storeName(displayName) {
  // Your code here
  document.cookie = `displayName=${displayName};max-age=30`;
}

// For restoring user's display name from cookies, if set in the past
function restoreName() {
  // Your code here
  let cookie = getCookie("displayName");
