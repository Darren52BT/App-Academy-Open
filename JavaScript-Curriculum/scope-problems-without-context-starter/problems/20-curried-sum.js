...
function curriedSum(numArgs) {
  ...
  let numbers = [];
  //inner function closes over numArgs and numbers array
  let sum;
  return sum = (num) => {
    //adds num to number array
    numbers.push(num);
    //if there are enough arguments, it adds all the numbers together and returns sum
    if (numbers.length >= numArgs) {
      return numbers.reduce((sum, ele) => sum + ele);
    } else { //if there aren't enough arguments, it returns the inner function 
      return sum;
    }
  }
}
