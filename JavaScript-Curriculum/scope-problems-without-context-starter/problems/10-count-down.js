...
const countDownTimer = n => {
  //if n is larger than 0, return inner function
  if (n > 0) {
    //inner function decrements n, if n is <= 0, return happy new year
    return () => {
      n--;
      if (n <= 0) {
        return "Happy New Year!";
      } else { //if n is still larger than 0, call countDownTimer with decremented n
        return countDownTimer(n);
      }

    }
  } else { //if n <= 0, return happy new year
    return "Happy New Year!";
  }
}
