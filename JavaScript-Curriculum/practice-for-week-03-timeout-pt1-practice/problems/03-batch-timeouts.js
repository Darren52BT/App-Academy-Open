...
function batchTimeouts(callbacks, delays) {
  // Your code here
  //iterative
  //return callbacks.map((cb, index) => setTimeout(cb, delays[index]));

  //recursive
  //base case of when there are no more callbacks
  if(callbacks.length === 0){
    return [];
  }
  //call setTimeout on the first callback and delay element, make a recursive call with the first elements removed
